...
fn smart_land_one_alert_goes_to_that_pane() {
    let result = amux::alert::smart_landing(
        &[false, true, false],
        2, 1,
    );
    assert_eq!(result, amux::alert::LandingTarget::FocusPane { index: 1, level: 2 });
}
...
fn smart_land_no_alerts_resumes() {
    ...
    assert_eq!(result, amux::alert::LandingTarget::Resume { level: 2, pane: 0 });
}
...
fn smart_land_was_at_level3_caps_at_level2() {
    ...
    assert_eq!(result, amux::alert::LandingTarget::Resume { level: 2, pane: 1 });
}
...
fn smart_land_was_at_level1_preserves() {
    ...
    assert_eq!(result, amux::alert::LandingTarget::Resume { level: 1, pane: 0 });
}
