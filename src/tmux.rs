...
/// 4. Zoom in (if needed) — handles pane cycling while zoomed: unzoom→select→re-zoom
/// 5. Dismiss alert
/// 6. Error message
...
pub fn execute_layout(session: &str, action: &crate::layout_engine::LayoutAction) -> Result<()> {
    ...
    if action.zoom == Some(true) {
        if is_zoomed(session).unwrap_or(false) {
            // Pane cycling while zoomed: already zoomed, need unzoom→select→re-zoom
...
/// Used by zoom-out from working mode.
